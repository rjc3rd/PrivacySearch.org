...
function load_opts()
{
    ...
    $opts->theme = $_REQUEST["theme"] ?? trim(htmlspecialchars($_COOKIE["theme"] ?? $opts->default_theme ?? "auto"));
...
function fetch_search_results($opts, $do_print)
{
    ...
    if (empty($results)) {
        require_once "engines/librey/fallback.php";
        $results = get_librey_results($opts);
    }
